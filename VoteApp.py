...
class Voting:
    def __init__(self):
        ...
        self.Vice_Head_Boy = ['SOYAM KUMAR S', 'NIKSHEEP GRAMPUROHIT', 'PRATHAM RAWAL']
        self.Vice_Head_Girl = ['PRAGYA MISHRA', 'SMRITI PREM', 'MAHIKA SURI']
        self.Head_Boy = ['BHAVVYA SARNA', 'DHRUV K', 'KEVIN CARVALHO']
        self.Head_Girl = ['SNEHA ROY', 'YASHASWINI GAUR', 'SAFREEN AFSAL']
        self.votes = []
        #data = {}
    ...
    def screen_scroll(self):
        ...
        if self.logged_in:
            ...
            for each in final_data:
                ...
                rw += 1
            #txt = '{} Won'.format(final_data[0][0].upper())
            #lbl = Label(self.enclosure, text=txt, font=heading, width=25, bg='light cyan',fg='SpringGreen4')
            #lbl.grid(row=rw, column=cl+1,columnspan=2)
            self.popup.update()
